use kosong_rs::{
    ...
    OpenAILegacyChatProvider, OpenAILegacyOptions, ProviderRequestAuth, Tool,
};
...
mod tests {
    ...
    fn text_stream_fixture() -> Fixture {
        ...
        serde_json::from_value(serde_json::json!({"systemPrompt":"","history":[{"role":"user","content":[{"type":"text","text":"Hi"}],"toolCalls":[]}],"providerOptions":{"model":"gpt-4o-mini","stream":true},"response":{"status":200,"stream":true,"body":body}})).unwrap()
    }
    #[tokio::test]
    ...
    async fn binary_reports_error_for_failed_response() {
        let fixture: Fixture = serde_json::from_value(serde_json::json!({"systemPrompt":"","history":[{"role":"user","content":[{"type":"text","text":"Hi"}],"toolCalls":[]}],"providerOptions":{"model":"gpt-4o-mini","stream":true},"response":{"status":401,"error":{"message":"Invalid auth"}},"expectError":true})).unwrap();
