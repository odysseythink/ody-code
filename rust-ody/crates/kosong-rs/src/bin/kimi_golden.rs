...
async fn run_fixture(fixture: Fixture) -> Result<Message, ChatProviderError> {
    ...
    let mut generation_kwargs = std::collections::HashMap::new();
    if let Some(effort) = fixture.provider_options.reasoning_effort {
        generation_kwargs.insert("reasoning_effort".into(), Value::String(effort));
    }
    if let Some(extra) = fixture.provider_options.extra_body {
        generation_kwargs.insert("extra_body".into(), Value::Object(extra));
    }
    ...
    let options = KimiOptions {
        ...
        stream: fixture.provider_options.stream,
        default_headers: None,
        generation_kwargs: Some(generation_kwargs),
