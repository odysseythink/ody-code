...
use crate::builtin::line_endings::{
    detect_line_ending_style, make_carriage_returns_visible, LineEndingStyle,
};
use crate::builtin::{
...
async fn execution(
    ...
    let header = match kaos
        .read_bytes(&safe_path, Some(MEDIA_SNIFF_BYTES as u64))
        .await
    {
        Ok(h) => h,
        Err(e) => {
...
mod tests {
    ...
    async fn run_read(tmp: &tempfile::TempDir, _path: &str, args: Value) -> String {
    ...
    async fn respects_line_offset_and_n_lines() {
        ...
        let out = run_read(
            &tmp,
            "a.txt",
            json!({"path": "a.txt", "line_offset": 2, "n_lines": 2}),
        )
        .await;
    ...
    async fn rejects_image_file() {
        ...
        tokio::fs::write(tmp.path().join("img.png"), &png_header)
            .await
            .unwrap();
        let kaos = Kaos::new(dummy_env(), tmp.path());
        let tool = ReadTool::new(kaos, workspace(tmp.path()));
        let exec = tool.resolve_execution(json!({"path": "img.png"})).unwrap();
        let result = (exec.execute)(ExecutableToolContext {
            turn_id: "".into(),
            tool_call_id: "".into(),
            signal: crate::builtin::AbortSignal::new(),
