...
export async function handleDesignCommand(host: SlashCommandHost, args: string): Promise<void> {
  const session = host.session;
  if (session === undefined) {
    host.showError(NO_ACTIVE_SESSION_MESSAGE);
    return;
  ...
  if (subcmd === 'clear') {
    await session.clearPlan();
    host.showNotice('Design cleared');
    return;
  }
  ...
  if (subcmd.length === 0) enabled = !(host.state.appState.designMode ?? false);
    ...
    host.showError(`Unknown design subcommand: ${subcmd}`);
    return;
  }

  await applyDesignMode(host, session, enabled);
}
...
async function applyDesignMode(host: SlashCommandHost, session: Session, enabled: boolean): Promise<void> {
  try {
    ...
    const msg = formatErrorMessage(error);
    host.showError(`Failed to set design mode: ${msg}`);
  }
}
