  ...
  private renderRecord(context: ReplayRenderContext, record: AgentReplayRecord): void {
    switch (record.type) {
      ...
      case 'session_mode_updated': {
        ...
        context.suppressNextPlanModeOffNotice = false;
        const isDesignRecord = record.kind === 'design';
        const modeLabel = isDesignRecord ? 'Design' : 'Plan';
        this.host.appendTranscriptEntry(
          replayEntry(context, 'status', `${modeLabel} mode: ${record.enabled ? 'ON' : 'OFF'}`, 'notice'),
        );
        return;
      }
