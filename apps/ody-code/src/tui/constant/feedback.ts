...
// at the `ody export` workflow so they can share diagnostics with us.
export function errorReportHintLine(sessionId: string): string {
  return `If this persists, run \`ody export ${sessionId}\` and share the file with us for diagnosis. Please don't share it publicly.`;
}
