...
export function pluginTrustLabel(plugin: PluginSummary): PluginTrustLabel {
  ...
  try {
    ...
    if (url.protocol !== 'https:' || url.hostname !== 'code.ody.com') {
      return 'third-party';
    }
    if (url.pathname.startsWith('/ody-code/plugins/official/')) {
      return 'official';
    }
    if (url.pathname.startsWith('/ody-code/plugins/curated/')) {
      return 'curated';
    }
    return 'third-party';
  } catch {
