 ...
 * conventions in `apps/ody-code/src/tui/components/dialogs/choice-picker.ts`.
 *
...
} from '@odysseythink/migration-legacy';
...
export interface MigrationScreenOptions {
   ...
   * now/later/never gate — used by the explicit `ody migrate` command, where
...
export class MigrationScreenComponent extends Container implements Focusable {
  ...
  constructor(opts: MigrationScreenOptions) {
    ...
    if (opts.skipDecisionStep === true) {
      // Explicit `ody migrate`: the now/later/never gate is meaningless, so
  ...
  private renderResult(width: number): string[] {
    ...
    if (this.migrationFailed) {
      ...
      lines.push('');
      lines.push(chalk.hex(colors.text)(' You can retry later by running "ody migrate".'));
      lines.push('');
