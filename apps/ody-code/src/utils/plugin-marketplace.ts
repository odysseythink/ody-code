...
export async function loadPluginMarketplace(
  ...
  const location = resolveMarketplaceLocation(
    options.source ?? process.env[ODY_CODE_PLUGIN_MARKETPLACE_URL_ENV] ?? ODY_CODE_PLUGIN_MARKETPLACE_URL,
    options.workDir,
  );
...
function resolveMarketplaceLocation(source: string, workDir: string): MarketplaceLocation {
  ...
  if (trimmed.length === 0) {
    throw new Error(`${ODY_CODE_PLUGIN_MARKETPLACE_URL_ENV} cannot be empty.`);
  }
