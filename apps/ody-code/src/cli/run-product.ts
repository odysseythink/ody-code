...
export async function runProduct(opts: CLIOptions, version: string): Promise<void> {
  ...
  log.info('kimi-code starting in product mode', {
    version,
    uiMode: CLI_UI_MODE,
  ...
  const tui = new OdyTUI(harness, {
    cliOptions: { ...opts, sessionMode: 'product', product: true },
    tuiConfig,
    version,
    workDir,
