...
function buildTarget(): string {
  return process.env['ODY_CODE_BUILD_TARGET'] ?? `${process.platform}-${process.arch}`;
}
...
export default defineConfig({
  ...
  define: {
    ...
    __ODY_CODE_VERSION__: JSON.stringify(packageJson.version),
    __ODY_CODE_CHANNEL__: JSON.stringify(process.env['ODY_CODE_CHANNEL'] ?? ''),
    __ODY_CODE_COMMIT__: JSON.stringify(process.env['ODY_CODE_COMMIT'] ?? ''),
    __ODY_CODE_BUILD_TARGET__: JSON.stringify(buildTarget()),
    __ODY_CODE_NATIVE_BUNDLE__: 'true',
  },
  deps: {
