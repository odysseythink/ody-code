...
assertIncludes(helpOutput, 'Usage: ody', '--help');

const exportHelpOutput = await runKimi(['export', '--help']);
assertIncludes(exportHelpOutput, 'Usage: ody export', 'export --help');

const nativeAssetOutput = await runKimiWithEnv(['--version'], {
  ODY_CODE_HOME: smokeHome,
  ODY_CODE_NATIVE_ASSET_SMOKE: '1',
});
