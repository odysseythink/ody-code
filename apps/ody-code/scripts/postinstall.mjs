 ...
 * ordering. The renamed shim is kept as `ody-legacy` so users can
 ...
 *     rename target like `ody.exe` → `ody-legacy.exe`).
 *
 * ## Code layout
 *
 ...
 *     `kimi`-vs-`ody-legacy` classification, the rename / unlink
 *     primitives.
 ...
 * ## Workflow
 *
 * What runs when a user types `npm install -g ody-code`
...
async function main() {
  ...
  // we can touch becomes `ody-legacy` (preserves what the user's
