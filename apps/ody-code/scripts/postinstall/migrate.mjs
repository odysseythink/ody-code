 ...
 *     `pathString` and returns every legacy `ody` shim along the
 ...
 *     decide whether an existing `ody-legacy` is itself a legacy CLI
 ...
 * with a misleading "ody now launches the new CLI" notice in front of
...
const LEGACY_BIN = 'ody';
const LEGACY_RENAME = 'ody-legacy';
...
/**
 * Expand `ody` into the set of filenames that resolve as executables
 * on this platform. POSIX → just `['ody']`. Windows → adds every
 * `PATHEXT` extension (so we find `ody.exe`, `ody.cmd`, etc).
 */
...
/**
 * Walk `pathString` and return every legacy `ody` shim along the
...
export async function detectLegacyShims(ownRoot, pathString) {
  ...
  for (const dir of pathEntries(pathString)) {
    for (const name of candidates) {
      ...
      // Defence-in-depth: never touch a `ody` that resolves into our
 ...
 * whether an existing `ody-legacy` is itself a legacy shim (safe to
 * drop the duplicate `ody`) or a user-managed file we must not
 * clobber.
 */
 ...
 * extension so a Windows `C:\…\ody.exe` ends up at `ody-legacy.exe`
 * rather than an extension-less `ody-legacy` that `ody.exe -- legacy`
 * shells won't run.
 */
 ...
 * which shim becomes ody-legacy) before any filesystem writes
 * happen.
 *
 ...
 *                        existing `ody-legacy` as the canonical
 ...
 *                        "preserve original ody" invariant fails for
 ...
 *   - we're consolidating onto an existing legacy `ody-legacy`, or
