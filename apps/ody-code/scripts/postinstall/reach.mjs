/**
 * Where the user actually types things, and what `ody` resolves to
 * from there.
 *
 * Covers:
 *
 ...
 *     `ody` into, not just the installer's environment.
 ...
 *     unknown/foreign `ody` (e.g. a user-written wrapper).
 *
...
/**
 * Expand a basename like `ody` into the set of filenames the OS
 * would actually match on PATH.
 *
 * On POSIX: just `['ody']`.
 *
 * On Windows: `['ody', 'ody.exe', 'ody.cmd', …]` — every
 * extension in `PATHEXT`. Without this, our PATH walk would miss
 * the typical `ody.exe` shim produced by `uv tool install` on
 * Windows.
 */
 ...
 *     --location=global ody-code` — verified on npm
 *     11.13.0.
 ...
 * directory so symlinked install layouts (e.g. `<prefix>/bin/ody`
...
//     do on POSIX)
const PACKAGE_NAME_MARKERS = ['ody-code', '@odysseythink\\kimi-code'];
 ...
 * `ody` into. A `~/.zshrc`-only PATH entry, a `~/.bash_profile`
...
export async function userShellPath() {
  ...
  return new Promise((resolve) => {
    ...
    const probe =
      'printf "<<<ODY_PATH_BEGIN>>>%s<<<ODY_PATH_END>>>\\n" "$PATH"';
    ...
    child.on('close', (code) => {
      ...
      const match = stdout.match(
        /<<<ODY_PATH_BEGIN>>>([\s\S]*?)<<<ODY_PATH_END>>>/,
      );
 ...
 *     because either source may contain a legacy `ody` we want to
 ...
 *     the user will actually type `ody` into. We do NOT union here:
