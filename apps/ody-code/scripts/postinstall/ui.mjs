 ...
 *     `ody-legacy`. The "`ody` now launches the new CLI" claim is
 *     safe to make here because the orchestrator already verified
 *     reachability after this set of removals.
 *   - "Consolidated" — first shim's `ody-legacy` already pointed at
 *     a legacy file (re-migration case); we deleted the duplicate
 *     source and kept the existing target. Same end state, different
 *     mechanism.
 *   - "Couldn't preserve as ody-legacy" — first shim's `ody-legacy`
 *     slot was a user-managed file; we deleted the source `ody` to
...
export function logMigrationDone(outcomes, pm) {
  const reinstallCmd = pmGlobalInstallCommand(pm, 'ody-code');
  const {
    renames,
    consolidates,
  ...
  const lines = [successHeading('ody now runs the new version'), ''];

  if (renames.length > 0) {
    lines.push(pad('   Renamed your old ody so you can still run it as'));
    lines.push(pad('   ody-legacy:'));
  ...
  if (consolidates.length > 0) {
    lines.push(pad('   Removed an extra copy of your old ody (ody-legacy'));
    lines.push(pad('   was already set up here from before):'));
    for (const c of consolidates) {
      lines.push(pathInBox(c.shimPath));
      lines.push(pathInBox('  (ody-legacy is at ' + c.target + ')'));
    }
    lines.push('');
  }

  if (skippedForeignTarget.length > 0) {
    lines.push(pad('   Removed your old ody (a file you created was already'));
    lines.push(pad('   using the name ody-legacy, so we left it alone):'));
  ...
  if (deletes.length > 0) {
    ...
    lines.push(pad('   new ody if we left them):'));
  ...
  if (blockedHarmless.length > 0) {
    ...
    lines.push(pad('   they won\'t run instead of the new ody:'));
  ...
  if (preservedSomewhere) {
    lines.push(
      pad('   Now typing `ody` runs the new version. To run the old'),
      pad('   version, type `ody-legacy` instead. Your settings from'),
      pad('   the old version will be moved over the first time you'),
      pad('   run `ody`.'),
      '',
      pad('   Note: if you reinstall the old ody later (e.g. with'),
      pad('   `uv tool`, `pip`, or `pipx`), it will put `ody` back.'),
    ...
    lines.push(
      pad('   Now typing `ody` runs the new version. Your settings'),
...
export function logMigrationBlocked(blocked, actionable, pm) {
  ...
  const lines = [
    warningHeading('Can\'t switch to the new ody yet'),
    '',
    pad('   There\'s an old ody on your computer that we can\'t change.'),
    pad('   As long as it\'s there, typing `ody` will still run the old'),
  ...
  if (actionable.length > 0) {
    lines.push(
      '',
      pad('   We also found these old ody files. We could remove them'),
...
/**
 * The reachability check found a `ody` ahead of our shim on PATH
...
export function logForeignKimiInTheWay(foreignPath, pm) {
  ...
  emit(
    renderBox([
      warningHeading('Can\'t switch to the new ody yet'),
      '',
      pad('   There\'s another file called `ody` on your computer that\'s'),
      pad('   not the new CLI and not the old one — it looks like'),
      pad('   something you set up yourself. As long as it\'s there,'),
      pad('   typing `ody` will run it instead of the new version.'),
      '',
      pad('   We found it at:'),
      pathInBox(foreignPath),
      '',
      pad('   To use the new ody, delete or rename that file, then'),
      pad('   install again:'),
      pathInBox(reinstallCmd),
      '',
      pad('   Nothing on your computer was changed.'),
...
/**
 * The legacy `ody` was found, but the directory where the package
 * manager placed the new `ody` shim is not on the user's PATH.
 * Renaming the legacy shim now would leave the user with NO reachable
 * `ody` command — the new one would still not be discoverable by
...
export function logNewCliNotOnPath(detection, pm) {
  ...
  emit(
    renderBox([
      warningHeading('New ody is installed, but your terminal can\'t find it'),
      '',
      pad('   The old ody is still here:'),
      pathInBox(detection.shimPath),
      '',
      pad('   The new ody was installed by ' + pm + ', but it landed in a'),
      pad('   folder your terminal doesn\'t search. (Your terminal looks'),
      pad('   for commands in folders listed in your PATH.) If we removed'),
      pad('   the old ody now, typing `ody` wouldn\'t find anything.'),
      '',
      pad('   Add the new ody\'s folder to your PATH (and save the change'),
      pad('   in your ' + rcLabel + ' so it sticks), then install again:'),
      pathInBox(newPathHint),
      pathInBox(reinstallCmd),
      '',
      pad('   The old ody is still where it was.'),
    ]),
  );
}
