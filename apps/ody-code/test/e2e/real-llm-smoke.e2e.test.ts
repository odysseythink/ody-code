/**
 * Real-LLM smoke. Opt-in only — runs when `ODY_E2E_REAL=1` is set, so
 ...
 *
 * Env knobs:
 *   ODY_E2E_REAL     — set to "1" to enable this suite
 *   ODY_E2E_MODEL    — model alias override (default: config's default)
 *   ODY_E2E_PROMPT   — prompt text (default: "Reply with a single word: hi")
 *   ODY_E2E_WORKDIR  — workspace directory (default: /tmp/kimi-e2e)
 */
...
const ENABLED = process.env['ODY_E2E_REAL'] === '1';
...
describe.skipIf(!ENABLED)('SDK e2e — real LLM smoke', () => {
  test(
    ...
    async () => {
      const workDir = process.env['ODY_E2E_WORKDIR'] ?? DEFAULT_WORKDIR;
      const prompt = process.env['ODY_E2E_PROMPT'] ?? DEFAULT_PROMPT;
      const modelAlias = process.env['ODY_E2E_MODEL'];
