...
describe('targetTriple', () => {
  ...
  it('honors ODY_CODE_BUILD_TARGET override', () => {
    expect(
      targetTriple({
        platform: 'darwin',
        arch: 'arm64',
        env: { ODY_CODE_BUILD_TARGET: 'linux-arm64' },
      }),
    ).toBe('linux-arm64');
  });
});
...
describe('executableName', () => {
  it('returns ody.exe on win32', () => {
    expect(executableName('win32')).toBe('ody.exe');
  });

  it('returns ody on other platforms', () => {
    expect(executableName('darwin')).toBe('ody');
    expect(executableName('linux')).toBe('ody');
  });
});
