...
const mocks = vi.hoisted(() => ({
  ...
  resolveOdyHome: vi.fn((homeDir?: string) => homeDir ?? '/tmp/kimi-export-home'),
...
describe('ody export', () => {
  ...
  it('parses --no-include-global-log as an option when no session id is given', async () => {
    ...
    await program.parseAsync(['node', 'ody', 'export', '--no-include-global-log', '-y']);
  ...
  it('initializes and flushes telemetry around default export tracking', async () => {
    ...
    await program.parseAsync(['node', 'ody', 'export', 'ses_telemetry', '--output', output], {
      from: 'node',
    });
  ...
  it('passes enabled false when default export config disables telemetry', async () => {
    ...
    await program.parseAsync(['node', 'ody', 'export', 'ses_disabled', '--output', output], {
      from: 'node',
    });
  ...
  it('tracks first launch around default export telemetry before harness construction can create the device id', async () => {
    ...
    await program.parseAsync(['node', 'ody', 'export', 'ses_first_launch', '--output', output], {
      from: 'node',
    });
