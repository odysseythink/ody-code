...
// Mock external modules used by runProduct
...
vi.mock('#tui/index', () => ({
  OdyTUI: vi.fn().mockImplementation(function () {
    const instance = {
      ...
      getCurrentSessionId: vi.fn().mockReturnValue('ses-product'),
    };
...
describe('runProduct', () => {
  it('creates harness and calls tui.start', async () => {
    ...
    // Should not throw
    await expect(runProduct(opts, '0.0.0-test')).resolves.toBeUndefined();
  });
  ...
  it('uses tui.getCurrentSessionId for office_hours_completed telemetry on exit', async () => {
    ...
    await runProduct(opts, '0.0.0-test');
    ...
    expect(mockWithTelemetryContext).toHaveBeenCalledWith({ sessionId: 'ses-product' });
  });
});
