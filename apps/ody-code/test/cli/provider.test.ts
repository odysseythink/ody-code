/**
 * `ody provider` CLI unit tests. The handlers receive an injected `getHarness`
...
import type { KimiConfig } from '@odysseythink/kimi-code-sdk';
...
describe('ody provider list', () => {
  ...
  it('renders one row per provider with counts and source labels', async () => {
    ...
    expect(out).toMatch(/managed:ody-code\s+type=kimi\s+models=0\s+source=oauth/);
...
describe('registerProviderCommand', () => {
  it('describes the user-facing subcommand and routes flags through commander', async () => {
    ...
    await tryRun(() =>
      program.parseAsync(
        ['node', 'ody', 'provider', 'add', REGISTRY_URL, '--api-key', 'sk-cli'],
        { from: 'node' },
      ),
    );
...
describe('ody provider catalog add', () => {
  ...
  it('rejects an unknown --default-model with a helpful hint', async () => {
    ...
    expect(err).toContain('ody provider catalog list anthropic');
  });
  ...
  it('does not persist default_thinking=false for first-time setup with --default-model', async () => {
    ...
    // explicit "off" request. A fresh `ody provider catalog add
