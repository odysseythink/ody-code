...
const mocks = vi.hoisted(() => {
  ...
  class TuiConfigParseError extends Error {
    ...
    constructor(fallback: TuiConfigFallback) {
      super('Invalid TUI config in ~/.ody-code/tui.toml; using defaults.');
  ...
  return {
    ...
    resolveOdyHome: vi.fn((homeDir?: string) => homeDir ?? '/tmp/kimi-code-test-home'),
...
vi.mock('@odysseythink/kimi-code-oauth', async () => {
  const actual = await vi.importActual<typeof import('@odysseythink/kimi-code-oauth')>(
    '@odysseythink/kimi-code-oauth',
  );
  return {
    ...actual,
    createKimiDeviceId: mocks.createKimiDeviceId,
    ODY_CODE_PROVIDER_NAME: 'kimi-code',
  };
});
...
describe('runShell', () => {
  afterEach(() => {
    ...
    mocks.resolveOdyHome.mockImplementation(
  ...
  it('detects auto theme and forwards config parse warnings as startup notice', async () => {
    ...
    expect(startupInput).toMatchObject({
      startupNotice: 'Invalid TUI config in ~/.ody-code/tui.toml; using defaults.',
      resolvedTheme: 'light',
      tuiConfig: {
        theme: 'auto',
        editorCommand: 'vim',
  ...
  it('tracks exit and prints resume instructions from the TUI exit handler', async () => {
    ...
    try {
      ...
      expect(stderr.text()).toContain(' To resume this session: ody -r ses-1');
    } finally {
      exitSpy.mockRestore();
      stdout.restore();
      stderr.restore();
  ...
  it('surfaces an invalid target config as an error for ody migrate, not silently', async () => {
    mocks.loadTuiConfig.mockResolvedValue({
      theme: 'dark',
      editorCommand: null,
      notifications: { enabled: true, condition: 'unfocused' },
    ...
    mocks.harnessGetConfig.mockRejectedValue(
      new Error('Invalid configuration in ~/.ody-code/config.toml'),
    );

    // A broken config.toml must fail loudly — `ody migrate` must not swallow
