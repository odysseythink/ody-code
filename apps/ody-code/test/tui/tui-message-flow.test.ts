...
const originalKimiCodeHome = process.env['ODY_CODE_HOME'];
const originalPluginMarketplaceUrl = process.env['ODY_CODE_PLUGIN_MARKETPLACE_URL'];
...
afterEach(async () => {
  ...
  if (originalKimiCodeHome === undefined) {
    ...
    process.env['ODY_CODE_HOME'] = originalKimiCodeHome;
  }
  ...
  if (originalPluginMarketplaceUrl === undefined) {
    delete process.env['ODY_CODE_PLUGIN_MARKETPLACE_URL'];
  } else {
    process.env['ODY_CODE_PLUGIN_MARKETPLACE_URL'] = originalPluginMarketplaceUrl;
  }
...
describe('KimiTUI message flow', () => {
  ...
  it('tracks theme changes from slash commands', async () => {
    process.env['ODY_CODE_HOME'] = await makeTempHome();
    const { driver, harness } = await makeDriver();
    harness.track.mockClear();
  ...
  it('shows the login prompt for auth.login_required session errors', async () => {
    ...
    expect(transcript).not.toContain('ody export');
  });

  it('appends the ody export hint beneath session error messages', async () => {
    const { driver } = await makeDriver();
    ...
    expect(transcript).toContain('If this persists, run `ody export ses-1`');
    expect(transcript).toContain("Please don't share it publicly");
  });

  it('skips the ody export hint when no active session id is set', async () => {
    const { driver } = await makeDriver();
  ...
  it('loads a local plugin marketplace file and installs from it', async () => {
    ...
    process.env['ODY_CODE_PLUGIN_MARKETPLACE_URL'] = marketplacePath;
    const session = makeSession();
    const { driver } = await makeDriver(session);
    ...
    try {
      ...
      await vi.waitFor(() => {
        expect(session.installPlugin).toHaveBeenCalledWith(
          'https://code.ody.com/ody-code/plugins/official/kimi-datasource.zip',
        );
      });
      expect(globalThis.fetch).toHaveBeenCalledWith(ODY_CODE_PLUGIN_MARKETPLACE_URL);
    } finally {
  ...
  it('applies /model selection with inline thinking state', async () => {
    ...
    expect(pickerOutput).toContain('Kimi K2 (Ody Code) ← current');
    expect(pickerOutput).toContain('❯ Kimi Turbo (Ody Code)');
    ...
    expect(filteredOutput).toContain('Kimi Turbo (Ody Code)');
    expect(filteredOutput).not.toContain('Kimi K2 (Ody Code)');
  ...
  it('enables search in the shared model selector helper', async () => {
    ...
    expect(output).toContain('Kimi Turbo (Ody Code)');
    expect(output).not.toContain('Kimi Alpha (Ody Code)');
  ...
  it('forks the active session and switches to the returned session', async () => {
    ...
    try {
      ...
      expect(driver.state.transcriptContainer.render(120).join('\n')).toContain(
        'Session forked (ses-fork). To return to the original session: ody -r ses-source',
      );
    } finally {
