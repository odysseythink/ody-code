...
function loginRequiredError(): Error & { readonly code: string } {
  return Object.assign(new Error('OAuth provider "managed:ody-code" requires login.'), {
...
describe("KimiTUI startup", () => {
  ...
  it("tracks login with already_logged_in when a token already exists", async () => {
    ...
    const harness = makeHarness(session, {
      auth: {
        status: vi.fn(async () => ({
          providers: [{ providerName: "managed:ody-code", hasToken: true }],
        })),
        login: vi.fn(async () => {}),
        logout: vi.fn(),
        getManagedUsage: vi.fn(),
  ...
  it("logs login failures with session context", async () => {
    ...
    const session = makeSession();
    const loginError = new Error("Failed to list Ody Code models (HTTP 402).");
    const harness = makeHarness(session, {
      auth: {
        status: vi.fn(async () => ({ providers: [] })),
        login: vi.fn(async () => {
      ...
      expect(warn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          error: expect.objectContaining({
            message: "Failed to list Ody Code models (HTTP 402).",
          }),
        }),
      );
    } finally {
  ...
  it("tracks logout after managed credentials and session state are cleared", async () => {
    ...
    const harness = makeHarness(session, {
      getConfig: vi.fn(async () => ({
        models: {
          k2: { provider: "managed:ody-code", model: "moonshot-v1", maxContextSize: 100 },
        },
        providers: { "managed:ody-code": { type: "kimi" } },
      })),
      auth: {
        status: vi.fn(async () => ({
          providers: [{ providerName: "managed:ody-code", hasToken: true }],
    ...
    expect(harness.auth.logout).toHaveBeenCalledWith("managed:ody-code");
  ...
  it("can log out a stale managed entry even after the OAuth token is gone", async () => {
    ...
    const harness = makeHarness(session, {
      ...
      auth: {
        ...
        status: vi.fn(async () => ({
          providers: [{ providerName: "managed:ody-code", hasToken: false }],
        })),
        login: vi.fn(async () => {}),
        logout: vi.fn(),
        getManagedUsage: vi.fn(),
  ...
  it("disposes terminal focus/theme tracking on the ody migrate exit", async () => {
    const harness = makeHarness();
    ...
    // `ody migrate` exits via process.exit; startEventLoop() installed focus
