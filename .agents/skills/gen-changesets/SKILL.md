...
3. **Do not list unchanged internal packages.** For example, if `packages/node-sdk` was not changed, do not list `@odysseythink/kimi-code-sdk` just because another internal package changed. The SDK follows the same rule as other internal packages: list it only when it was actually changed.
4. **Internal package source changes that enter the CLI bundle must manually list the CLI.** `@odysseythink/kimi-code` inline-bundles `@odysseythink/*` source, but those internal packages are devDependencies from the CLI's perspective, so changesets will not automatically propagate bumps. If a change enters the CLI output, also list `@odysseythink/kimi-code`.
...
6. `@odysseythink/vis` / `vis-server` / `vis-web` are ignored by changesets and should not be handled.

## Workflow

1. List the packages that were actually changed.
2. Choose a bump level for each package.
3. If an internal package change enters the CLI bundle, add `@odysseythink/kimi-code`.
...
```markdown
---
"@odysseythink/kimi-code-sdk": patch
---
...
- Internal package source enters the CLI bundle, but `@odysseythink/kimi-code` is missing.
- `packages/node-sdk` was not changed, but `@odysseythink/kimi-code-sdk` was listed for "internal package sync".
