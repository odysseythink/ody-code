...
- `apps/ody-code`: the CLI / TUI application. It consumes core capabilities through `@odysseythink/kimi-code-sdk` and must not depend directly on `@odysseythink/agent-core`.
...
- Gate a not-yet-public feature behind an experimental flag. Add the flag to the registry at `packages/agent-core/src/flags/registry.ts`, then check it with `flags.enabled('my-feature')`. Flags are env-driven and default off: `ODY_CODE_EXPERIMENTAL_<NAME>` toggles one, `ODY_CODE_EXPERIMENTAL_FLAG` enables all. Release by flipping the entry's `default` to `true`.
