...
- Modify: `rust-ody/crates/ody-crypto/src/lib.rs`
- Modify: `rust-ody/crates/ody-crypto/Cargo.toml`（`rsa` dev-dep 已在 Task 1 中声明，无需重复）
...
```bash
node -e "const addon = require('./packages/ody-crypto-' + process.platform + '-' + process.arch + '/ody-crypto.node'); console.log(addon.sha256('abc'))"
```
