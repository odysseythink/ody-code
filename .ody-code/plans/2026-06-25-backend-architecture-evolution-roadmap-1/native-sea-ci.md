...
packages/ody-crypto/scripts/build-native.sh           # npm target -> rust triplet wrapper
...
**Depends on:** `2026-06-25-backend-architecture-evolution-roadmap-1/ts-loader.md`: Task 7、Task 8（平台子包与宿主包已创建）
...
**Depends on:** `2026-06-25-backend-architecture-evolution-roadmap-1/ts-loader.md`: Task 8（`packages/ody-crypto` 已创建）

**Files：**
- Modify: `packages/ody-crypto/package.json`（追加 `build:native` 脚本）
- Create: `packages/ody-crypto/scripts/build-native.sh`
- Modify: `package.json:7-28`（追加 root `build:native:crypto` 脚本）

- [ ] 创建 `packages/ody-crypto/scripts/build-native.sh`：
...
- [ ] 提交：`git add packages/ody-crypto/package.json packages/ody-crypto/scripts/build-native.sh package.json && git commit -m "chore(ody-crypto): add build:native:crypto script"`
