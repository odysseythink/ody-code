...
3. 平台子包 `packages/ody-crypto-<target>`(darwin-arm64/x64、linux-arm64/x64、win32-x64),每个只含对应 `.node`;`win32-arm64` 暂不入列(见 Out)。
...
- 不追求跨平台 `win32-arm64` 与 `linux-musl`;目标矩阵与现有 `SUPPORTED_TARGETS` 一致,先聚焦于 SEA 已支持的五平台(darwin-arm64/x64、linux-arm64/x64、win32-x64)。
...
#[napi]
pub fn verify_id_token(jwt: String, jwk_json: String, expected_issuer: String, expected_audience: String) -> Result<IdTokenClaims, Error>;
```
...
| 错误类 | 立即处理 | 降级路径 | 恢复条件 |
|---|---|---|---|
| Native `.node` require 失败(dlopen 不兼容、SEA asset 缺失) | 捕获并 debug log | 返回 TS fallback(`node:crypto` + `pkce-challenge` + `jose`) | 下一次进程启动重试;若 SEA asset 修复则自动恢复 |
...
- 本地开发:安装依赖时,pnpm 通过 `optionalDependencies` 只安装当前平台子包;非当前平台子包可 `optionalDependencies` + `os`/`cpu` 字段,缺失不中断安装。
- CI:新增 `build:native:crypto` 脚本,在对应 runner 上为每个 target 编译 `.node`,并复制到平台子包目录。GitHub Actions 矩阵:macos-13(x64)、macos-latest(arm64)、ubuntu-latest(x64)、ubuntu-24.04-arm(arm64)、windows-latest(x64)。
...
- SEA smoke test passes on CI for darwin-arm64/x64、linux-arm64/x64、win32-x64
...
| A4 | Rust `jsonwebtoken` crate 支持用 JWK(JSON string) 直接验证 RS256/ES256,无需额外 PEM 转换。 | 中 | TS 侧需要先把 JWK 转 PEM,增加边界复杂度和失败点。 |  spike:用 `jsonwebtoken` 单元测试验证 JWK 输入。 |
| A5 | 平台子包用 optionalDependencies + `os`/`cpu` 字段不会破坏非对应平台的 `pnpm install --frozen-lockfile`。 | 中 | 开发/CI 安装失败。 | 在子包 package.json 中设置 `os`/`cpu`,并在非目标平台 runner 上测试 install。 |
...
1. **Native loader 的平台映射决定某平台是走 Native 还是 TS fallback。**
   - 用例 A:`process.platform='darwin', process.arch='arm64'` → 目标 `darwin-arm64`,Native 可用。
   - 用例 B:`process.platform='win32', process.arch='arm64'` → 目标 `win32-arm64`,不在 `SUPPORTED_TARGETS`,返回 null 走 TS fallback。
   - 用例 C:`process.platform='linux', process.arch='ia32'` → 目标 `linux-ia32`,不在支持列表,返回 null 走 TS fallback。
   - 已用 `node -e` 验证 `${platform}-${arch}` 与现有 `SUPPORTED_TARGETS` 五项完全对齐。
...
3. **`verifyIdToken` 的 TS fallback 依赖 `jose` 库支持 JWK RS256/ES256。**
   - 用例 A:合法 RSA JWK + RS256 JWT → `jose` 应返回 claims。
   - 用例 B:过期 JWT → `jose` 应抛出 `JWTExpired`。
   - 用例 C:签名被篡改 → `jose` 应抛出 `JWSSignatureVerificationFailed`。
   - 由于 `jose` 尚未安装,此假设需 spike 验证;若失败,则 fallback 方案改为"JWT 校验仅在 Native 可用,失败时跳过校验并记录 warning"。

**四透镜扫描**:

- **Security**:检查了 `verifyIdToken` 的 alg 白名单、aud 处理(支持字符串与数组)、exp 校验、敏感值不打印。未发现 secrets 泄漏到日志;修正:明确拒绝 `alg=none` 与 HS256。
- **Test**:每个行为都有 must-pass/must-reject 断言;测试表已列出。修正:补充了 `win32-arm64` 不在支持列表的 fallback 测试用例。
