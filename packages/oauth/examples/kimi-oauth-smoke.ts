...
async function main(): Promise<void> {
  ...
  try {
    if (forceLogin) {
      await toolkit.logout(ODY_CODE_PROVIDER_NAME);
      process.stdout.write('cleared existing smoke token\n');
    }

    const login = await toolkit.login(ODY_CODE_PROVIDER_NAME, {
      onDeviceCode: printDeviceCode,
    });
    const status = await toolkit.status(ODY_CODE_PROVIDER_NAME);
    const accessToken = await toolkit.tokenProvider(ODY_CODE_PROVIDER_NAME).getAccessToken();
    const usage = await toolkit.getManagedUsage(ODY_CODE_PROVIDER_NAME);
    ...
    if (config.providers[ODY_CODE_PROVIDER_NAME] === undefined) {
    ...
    if (!keepToken) {
      await toolkit.logout(ODY_CODE_PROVIDER_NAME).catch(() => {});
    }
...
function smokeIdentityFromEnv(): KimiHostIdentity {
  ...
  if (version === undefined || version.trim().length === 0) {
    throw new Error('ODY_CODE_SMOKE_VERSION is required for Kimi OAuth smoke.');
  }
  return {
