...
describe('Kimi identity factories', () => {
  ...
  it('creates complete X-Msh device headers from host version', () => {
    ...
    expect(headers['X-Msh-Platform']).toBe(ODY_CODE_PLATFORM);
  ...
  it('creates ody-code-cli User-Agent and appends suffix only to UA', () => {
    expect(
      createKimiUserAgent({
        userAgentProduct: 'ody-code-cli',
        version: '1.2.3',
      }),
    ).toBe('ody-code-cli/1.2.3');
    expect(
      createKimiUserAgent({
        userAgentProduct: 'ody-code-cli',
        version: '1.2.3',
    ...
    ).toBe('ody-code-cli/1.2.3 (wire 4.5.6)');
  });
  ...
  it('merges User-Agent and device headers into default headers', () => {
    ...
    expect(headers['User-Agent']).toBe('ody-code-cli/1.2.3');
...
describe('ascii header value sanitization', () => {
  it('strips a trailing newline from a header value', () => {
    const ua = createKimiUserAgent({ userAgentProduct: 'ody-code-cli', version: '6.8.0-101\n' });
    expect(ua).toBe('ody-code-cli/6.8.0-101');
  });

  it('drops non-ASCII codepoints while keeping the ASCII remainder', () => {
    const ua = createKimiUserAgent({ userAgentProduct: 'ody-code-cli', version: 'héllo' });
    expect(ua).toBe('ody-code-cli/hllo');
  });
