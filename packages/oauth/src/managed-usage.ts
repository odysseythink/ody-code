 ...
 *
 * Only `managed:ody-code` is supported today. The platform exposes a
...
const ODY_CODE_PLATFORM_ID = 'ody-code';
...
export function isManagedKimiCode(providerKey?: string | null): boolean {
  ...
  return providerKey.slice(MANAGED_PREFIX.length) === ODY_CODE_PLATFORM_ID;
}

export function kimiCodeBaseUrl(): string {
  return process.env['ODY_CODE_BASE_URL'] ?? 'https://api.kimi.com/coding/v1';
}
