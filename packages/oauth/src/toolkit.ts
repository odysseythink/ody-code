...
import { ODY_CODE_FLOW_CONFIG } from './constants';
...
export class KimiOAuthToolkit<TConfig = unknown> {
  ...
  constructor(options: KimiOAuthToolkitOptions<TConfig>) {
    ...
    this.flowConfig = options.flowConfig ?? ODY_CODE_FLOW_CONFIG;
  ...
  async status(providerName?: string | undefined): Promise<AuthStatus> {
    const name = providerName ?? ODY_CODE_PROVIDER_NAME;
    return {
      providers: [
        {
          providerName: name,
  ...
  async logout(providerName?: string | undefined): Promise<KimiOAuthLogoutResult> {
    ...
    if (this.configAdapter?.remove !== undefined && name === ODY_CODE_PROVIDER_NAME) {
  ...
  async getCachedAccessToken(
    ...
    const name = providerName ?? ODY_CODE_PROVIDER_NAME;
    const oauthKey = oauthRef?.key ?? ODY_CODE_OAUTH_KEY;
  ...
  managerFor(providerName: string, oauthKey = ODY_CODE_OAUTH_KEY): OAuthManager {
...
export function resolveKimiTokenStorageName(input: {
  ...
  const providerName = input.providerName ?? ODY_CODE_PROVIDER_NAME;
  if (providerName !== ODY_CODE_PROVIDER_NAME) {
    throw new Error(`No OAuth manager configured for provider "${providerName}".`);
  }

  const key = input.oauthKey ?? ODY_CODE_OAUTH_KEY;
  if (key ===  'ody-code' || key === ODY_CODE_OAUTH_KEY) return  'ody-code';
...
function defaultKimiHome(): string {
  const override = process.env['ODY_CODE_HOME'];
