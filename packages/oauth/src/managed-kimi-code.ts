...
export const ODY_CODE_PLATFORM_ID = 'ody-code';
export const ODY_CODE_PROVIDER_NAME = 'managed:ody-code';
export const ODY_CODE_OAUTH_KEY = 'oauth/ody-code';
...
export interface ManagedKimiCodeProvisionResult {
  readonly providerName: typeof ODY_CODE_PROVIDER_NAME;
...
export interface ManagedKimiOAuthRef {
  ...
  readonly key: typeof ODY_CODE_OAUTH_KEY;
}
...
function managedModelKey(modelId: string): string {
  return `${ODY_CODE_PLATFORM_ID}/${modelId}`;
}
...
function toModelInfo(item: unknown): ManagedKimiCodeModelInfo | undefined {
  ...
  if (!Number.isInteger(contextLength) || contextLength <= 0) {
    throw new Error(`Ody Code model "${item['id']}" must include a positive context_length.`);
  }
  ...
  if (!response.ok) {
    throw new Error(
      await readApiErrorMessage(
        response,
        `Failed to list Ody Code models (HTTP ${response.status}).`,
      ),
    );
  }
...
export function applyManagedKimiCodeConfig(
  ...
  if (options.models.length === 0) {
    throw new Error('No models available for Ody Code.');
  }
  ...
  config.providers[ODY_CODE_PROVIDER_NAME] = {
    type: 'kimi',
    baseUrl,
    apiKey: '',
    oauth: { storage: 'file', key: ODY_CODE_OAUTH_KEY },
  };

  for (const [key, model] of Object.entries(existingModels)) {
    if (isRecord(model) && model['provider'] === ODY_CODE_PROVIDER_NAME) {
...
export function applyManagedKimiCodeLogoutConfig(config: ManagedOdyConfigShape): void {
  ...
  const existingModels = config.models ?? {};
  for (const [key, model] of Object.entries(existingModels)) {
    if (!isRecord(model) || model['provider'] !== ODY_CODE_PROVIDER_NAME) continue;
    delete existingModels[key];
  ...
  if (config['defaultProvider'] === ODY_CODE_PROVIDER_NAME) {
...
function canPreserveDefaultModel(
  ...
  return isRecord(existing) && existing['provider'] !== ODY_CODE_PROVIDER_NAME;
}
...
function assertPositiveContextLength(model: ManagedKimiCodeModelInfo): void {
  if (!Number.isInteger(model.contextLength) || model.contextLength <= 0) {
    throw new Error(`Ody Code model "${model.id}" must include a positive context_length.`);
  }
}
