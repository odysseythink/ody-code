...
export const AppendBuilderProfileInputSchema = z.object({
  ...
  designDoc: z.string().optional().describe('Path to the design document produced during Phase 5. Defaults to the current product design file path if omitted.'),
    ...
    return {
      ...
      execute: async () => {
        ...
        try {
          ...
          await this.agent.productStateStore.appendProfile(entry);
          return {
            output: t('product.profileAppended', this.agent.userLanguage),
          };
        } catch (error) {
