...
- **product**: YC office hours diagnostic workflow. Activates when the user explicitly requests office hours or asks for startup/builder diagnostic help.
      ...
      execute: async () => {
        ...
        try {
          ...
          if (!fileExists) {
            ...
            return { output: t('product.agentsMdCreated', this.agent.userLanguage).replace('{path}', claudeMdPath) };
          }

          if (content!.includes('## Skill routing')) {
            return { output: t('product.agentsMdAlreadyHasRouting', this.agent.userLanguage) };
          }

          const updated = content!.trimEnd() + '\n' + ROUTING_SECTION;
          await this.agent.kaos.writeText(claudeMdPath, updated);
          return { output: t('product.agentsMdUpdated', this.agent.userLanguage).replace('{path}', claudeMdPath) };
        } catch (error) {
          const message = error instanceof Error ? error.message : 'Failed to ensure AGENTS.md routing.';
          return { isError: true, output: t('product.failedToEnsureRouting', this.agent.userLanguage).replace('{message}', message) };
        }
      },
    };
  }
