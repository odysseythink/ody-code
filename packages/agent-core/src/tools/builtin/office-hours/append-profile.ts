    ...
    return {
      ...
      execute: async () => {
        if (!this.agent.sessionMode.isActive || this.agent.sessionMode.kind !== 'office-hours') {
          return {
            isError: true,
            output: 'Office hours mode is not active. AppendBuilderProfile is only available during office hours sessions.',
          };
        }
        ...
        try {
          ...
          return {
            output: `Builder profile entry appended successfully. Session count will be updated for next tier computation.`,
          };
        } catch (error) {
