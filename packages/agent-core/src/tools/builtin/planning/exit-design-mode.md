...
- **Reuse Analysis** — a `## Reuse Analysis` section listing existing-code reuse candidates (or an explicit greenfield note). English alternatives such as `## Component Reuse` or `## Existing Components` are also accepted.
