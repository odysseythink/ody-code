...
`kimi-code` uses changesets for versioning. Each package gets its own `CHANGELOG.md`. The user-facing CLI package, `@odysseythink/kimi-code`, writes its changelog here:
...
- The released version exists on npm (`npm view @odysseythink/kimi-code versions --json`) or has a matching GitHub Release tag.
...
- The upstream H1 `# @odysseythink/kimi-code` because the docs page already has `# Changelog`.
...
This page documents the changes in each Ody Code CLI release.
```
