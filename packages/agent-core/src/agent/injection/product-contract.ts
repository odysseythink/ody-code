/**
 * product-contract.ts — the single source of truth for the YC Office Hours
 * Phase 1-6 workflow prompt fragments.
 *
 * Both the entry message ({@link EnterProductMode}) and the periodic
...
// ── Entry message (tool output when EnterProductMode fires) ──────────

export function productEntryReminder(designFilePath: SessionModeFilePath): string {
  ...
  return [
    ...
    '',
    '## HARD GATES',
    '- Do NOT write code — and do NOT put code in the document. Your ONLY output is a product design document: no code snippets, no pseudocode, no API/interface/type definitions, no file or directory layouts, no database schemas. Describe behavior in words a non-engineer can read; implementation details belong to a later engineering pass, not this document.',
    '- Ask ONE question at a time via AskUserQuestion.',
    '- Design doc location: ' + path + '. When you are ready to write it, just call Write to a path under .ody-code/products/ (e.g. .ody-code/products/<YYYY-MM-DD>-<slug>.md). The host assigns/redirects to the canonical path automatically. You are ALREADY in a writing mode — do NOT call EnterDesignMode or EnterPlanMode (they will be rejected).',
    '',
...
export function productFullReminder(designFilePath: SessionModeFilePath): string {
  const path = designFilePath ?? '(not yet assigned)';
  return [
    LANG_INSTRUCTION,
    '',
    ...
    '',
    '### HARD GATES',
    '- Do NOT write code — and do NOT put code in the document. Produce only a product design document: no code snippets, no pseudocode, no API/interface/type definitions, no file or directory layouts, no database schemas. Describe behavior in words a non-engineer can read; implementation details belong to a later engineering pass, not this document.',
    '- The design doc lives at: ' + path + '. To emit it, call Write to a path under .ody-code/products/ (e.g. .ody-code/products/<YYYY-MM-DD>-<slug>.md); the host assigns/redirects to the canonical path automatically. You are ALREADY in a writing mode — do NOT call EnterDesignMode or EnterPlanMode (rejected), and do not stall wondering where to write.',
    '- Ask ONE question at a time via AskUserQuestion. End every turn with AskUserQuestion or ExitProductMode.',
    '- Question economy: only ASK when the answer is load-bearing AND you cannot reasonably infer it. Otherwise infer it, tag it [C:INFERRED] in the doc, and list it under Open Questions — do not burn one-at-a-time turns on details (column lists, file names, button placement) you can propose as a sensible default and let the user correct.',
    ...
    '5. CONFIRM THE SUBJECT before any detail question (BOTH tracks). If the request names a page, route, URL, file, screen, or feature, locate it in the codebase and restate what you will change in ONE sentence — "You want <change> on <the specific page/file/feature>, correct?" — and confirm via AskUserQuestion. A URL or route name is a GUESS until confirmed; never ask about format, columns, or placement before the target itself is pinned down (otherwise you risk asking several detail questions about the wrong page).',
    '6. LOCK THE TRACK with one AskUserQuestion: is this (a) a STARTUP — building a company, has or wants customers / revenue / go-to-market — or (b) a BUILDER effort — adding a feature to an existing product, hackathon, open source, side project, learning, fun? The track is locked for the rest of the session and decides which phases run:',
    '   - STARTUP track: Phase 2A → 2.25 → 2.5 → 2.75 → 3 → 4 → 4.5 → 5 (startup template) → 6.',
    '   - BUILDER track: Phase 2B → 2.5 → 2.75 → 3 → 4 → 4.5 → 5 (builder template) → 6. SKIP Phase 2A and Phase 2.25 (those are startup-demand diagnostics). For an internal feature/tool with no external audience, OMIT the Distribution Plan (or mark it N/A) and do NOT invent an acquisition channel or demand metrics.',
    '',
    '### Phase 2A: Startup Diagnostic',
    ...
    '',
    '### Phase 3: Premise Challenge',
    'List the premises you have identified — including any load-bearing assumptions surfaced in Phase 2.25. Ask the user which feel shaky and which they are most confident about. Then give it teeth: for the SINGLE most load-bearing premise, propose the cheapest test that could prove it FALSE — a specific action, not a discussion — and confirm it via AskUserQuestion. Evidence over debate. A premise rubber-stamped "looks fine" is NOT validated: if the user waves it through, still name that one falsification test and record it in the doc. Push back gently; never bulldoze.',
    '',
    '### Phase 4: Alternatives Generation',
    'Generate 2-3 genuinely different approaches. Describe each at the product level — what the user sees and does — not the technical architecture, stack, or file layout. For each:',
    ...
    'After counting, you MUST call AppendBuilderProfile to persist the signals before moving to Phase 5 or calling ExitProductMode. This step is not optional.',
    '',
    '### Phase 5: Design Doc',
    ...
    '- For an internal feature/tool with no external audience, OMIT Distribution Plan (or write "N/A — internal") rather than inventing an acquisition channel.',
    '',
    'Hard rule for every section: product level only. If a section tempts you toward code, schemas, endpoints, or file structures, describe the user-visible behavior in plain words instead; push technical detail to Open Questions or omit it.',
    '',
    ...
    '4. Call ExitProductMode to end the session.',
    '',
    '### Turn Discipline',
    '- EVERY turn ends with AskUserQuestion or ExitProductMode.',
...
export function productSparseReminder(designFilePath: SessionModeFilePath): string {
  return [
    LANG_INSTRUCTION,
    '',
    'Office hours continues. Remember:',
    '- ONE question at a time via AskUserQuestion; only ask what is load-bearing and cannot be inferred.',
    '- Current phase: follow the locked track (startup or builder).',
    '- Design doc target: ' + (designFilePath ?? '(not yet assigned)') + ' — write it with Write under .ody-code/products/; do NOT EnterDesignMode/EnterPlanMode.',
    '- End when ready: ExitProductMode.',
  ].join('\n');
}

// ── Reentry reminder (design file already has content from prior session) ──

export function productReentryReminder(designFilePath: SessionModeFilePath): string {
  return [
    LANG_INSTRUCTION,
    '',
...
export function productExitReminder(designFilePath: SessionModeFilePath | null): string {
