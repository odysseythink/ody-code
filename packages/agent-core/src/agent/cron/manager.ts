 ...
 * `ODY_CRON_NO_STALE=1` (bench / acceptance tests).
 *
...
export interface CronManagerOptions {
   ...
   * `resolveClockSources(process.env.ODY_CRON_CLOCK)` so production
   * picks up `ODY_CRON_CLOCK=file:...` automatically.
   ...
   * (1000ms unless `ODY_CRON_MANUAL_TICK=1`, which forces `null` here
...
export class CronManager {
  ...
  constructor(agent: Agent, opts: CronManagerOptions = {}) {
    ...
    this.clocks =
      opts.clocks ??
      resolveClockSources(process.env['ODY_CRON_CLOCK']) ??
      SYSTEM_CLOCKS;
    this.persistStore =
    ...
    this.scheduler = createCronScheduler({
      ...
      isKilled: () => process.env['ODY_DISABLE_CRON'] === '1',
      onFire: (task, ctx) => {
      ...
      // P1.8: `ODY_CRON_MANUAL_TICK=1` forces the scheduler into
   ...
   * Stale judgment.
   *
   *   - `ODY_CRON_NO_STALE=1` short-circuits to false (bench).
  ...
  isStale(task: CronTask): boolean {
    if (process.env['ODY_CRON_NO_STALE'] === '1') return false;
   ...
   *
   * Gated on `ODY_CRON_MANUAL_TICK=1` for two reasons:
   *
   ...
   * Set `ODY_CRON_DEBUG=1` to surface the swallowed error to stderr —
   * mirrors `scheduler.ts`'s debugLog pattern so bench debugging can
   * see a bad tick.
   */
  private bindSigusr1(): void {
    if (process.platform === 'win32') return;
    if (process.env['ODY_CRON_MANUAL_TICK'] !== '1') return;
    if (this.sigusr1Handler !== null) return;
    const handler: NodeJS.SignalsListener = () => {
      try {
        this.tick();
      } catch (error) {
        if (process.env['ODY_CRON_DEBUG'] === '1') {
