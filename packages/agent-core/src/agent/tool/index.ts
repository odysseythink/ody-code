...
export class ToolManager {
  ...
  *toolInfos(): Iterable<ToolInfo> {
  ...
  /**
   * Whether a builtin/user tool is currently visible to the model — i.e. both
   * enabled by the active profile AND actually registered. Conditionally
   * registered tools (e.g. ShowDesignMockup, gated on `rpc.openExternal`) are
   * only in `builtinTools` when the host supports them, so this single check
   * covers both "profile enabled it" and "the host can provide it". Used by the
   * design-mode injector so the prompt never advertises a tool the model can't
   * see.
   */
  isToolActive(name: string): boolean {
    return (
      this.enabledTools.has(name) &&
      (this.builtinTools.has(name) || this.userTools.has(name))
    );
  }
  ...
  initializeBuiltinTools() {
    ...
    this.builtinTools = new Map(
      [
        ...
        new b.SetProductLanguageTool(this.agent),
        new b.EnterProductModeTool(this.agent),
        new b.ExitProductModeTool(this.agent),
        ...
        new b.SyncProductArtifactTool(this.agent),
