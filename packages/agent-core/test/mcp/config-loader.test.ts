...
describe('resolveMcpJsonPaths', () => {
  it('returns the canonical user and project paths', () => {
    const paths = resolveMcpJsonPaths({ cwd: '/work/proj', homeDir: '/home/user/.ody-code' });
    expect(paths.user).toBe('/home/user/.ody-code/mcp.json');
    expect(paths.project).toBe('/work/proj/.ody-code/mcp.json');
  });
});
...
describe('loadMcpServers', () => {
  ...
  it('merges project-local mcp.json with user-global, project overriding on conflict', async () => {
    ...
    await writeJson(join(cwd, '.ody-code', 'mcp.json'), {
      mcpServers: {
  ...
  it('honors ODY_CODE_HOME env var when homeDir is not supplied', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
    ...
    process.env['ODY_CODE_HOME'] = home;
    try {
