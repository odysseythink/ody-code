...
import {
  ...
  productExitReminder,
} from '#agent/injection/product-contract';

describe('product-contract', () => {
  const path = '/project/.ody-code/product/2026-06-16-my-startup.md';
  ...
  describe('productFullReminder', () => {
    ...
    it('requires AppendBuilderProfile before exit', () => {
      ...
      expect(msg).toContain('before moving to Phase 5 or calling ExitProductMode');
    });
  ...
  describe('productSparseReminder', () => {
    it('is shorter than full reminder', () => {
      const sparse = productSparseReminder(path);
      const full = productFullReminder(path);
      expect(sparse.length).toBeLessThan(full.length);
    });

    it('includes ONE question at a time', () => {
      expect(productSparseReminder(path)).toContain('ONE question');
    });
  });

  describe('productExitReminder', () => {
    it('signals session completion', () => {
      expect(productExitReminder(path)).toContain('complete');
    });
  });

  describe('productReentryReminder', () => {
    it('acknowledges existing content', () => {
      expect(productReentryReminder(path)).toContain('existing');
    });
  });
});
