...
import { inputTotal } from '@odysseythink/kosong';
...
describe('runTurn — turn lifecycle', () => {
  ...
  it('throws OdyError(loop.max_steps_exceeded) when steps reach maxSteps', async () => {
    const echo = new EchoTool();
    ...
    expect((error as OdyError).code).toBe(ErrorCodes.LOOP_MAX_STEPS_EXCEEDED);
    expect((error as OdyError).details).toEqual({ maxSteps: 2 });
