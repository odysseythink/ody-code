...
describe('AskUserQuestionTool', () => {
  ...
  it('starts a background question task and stores the eventual answer in task output', async () => {
    vi.stubEnv('ODY_CODE_EXPERIMENTAL_FLAG', '1');
  ...
  it('downgrades background=true to an inline question when the experimental flag is off', async () => {
    vi.stubEnv('ODY_CODE_EXPERIMENTAL_FLAG', '0');
    vi.stubEnv('ODY_CODE_EXPERIMENTAL_BACKGROUND_ASK', '0');
  ...
  it('resolves question rpc error responses as dismissed answers', async () => {
    const { tool } = makeTool({
      requestQuestion: async () => {
        throw new OdyError(ErrorCodes.INTERNAL, 'JSON-RPC question error response');
      },
    });
  ...
  it('returns a distinct hard error when the client signals unsupported', async () => {
    const { tool } = makeTool({
      requestQuestion: async () => {
        throw new OdyError(ErrorCodes.NOT_IMPLEMENTED, 'Client does not support questions');
      },
    });
