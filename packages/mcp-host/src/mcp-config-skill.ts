/**
 * Export a SkillDefinition-compatible constant for the `mcp-config` builtin skill.
 * This lives in mcp-host because the skill is MCP-specific.
 * agent-core imports and registers it as a builtin skill.
 *
...
interface McpConfigSkillDefinition {
  ...
  readonly plugin?: { readonly id: string } | undefined;
  readonly mermaid?: string | undefined;
  readonly d2?: string | undefined;
  readonly metadata: {
...
const CONTENT = `# Interactive MCP server configuration
...
- User-global: \`~/.ody-code/mcp.json\` (or \`$ODY_CODE_HOME/mcp.json\` if
  set). Use for servers you want everywhere.
- Project-local: \`<cwd>/.ody-code/mcp.json\`. Mention once that stdio
...
export const MCP_CONFIG_SKILL: McpConfigSkillDefinition = {
  name: 'mcp-config',
