...
import { resolveOdyHome } from './paths';
...
export function resolveMcpJsonPaths(input: ResolveMcpJsonPathsInput): McpJsonPaths {
  return {
    user: join(resolveOdyHome(input.homeDir), 'mcp.json'),
    project: join(input.cwd, '.ody-code', 'mcp.json'),
  };
}
...
/**
 * Load MCP server declarations from the user-global `~/.ody-code/mcp.json`
 * and the project-local `<cwd>/.ody-code/mcp.json`. Entries in the project
