...
import type { Tool } from '@odysseythink/kosong';

import { abortable } from '@odysseythink/agent-core-shared';
...
export class McpConnectionManager {
  ...
  constructor(private readonly options: McpConnectionManagerOptions = {}) {
    ...
    this.log = options.log ?? fallbackLogger;
  }
  ...
  async reconnect(name: string): Promise<void> {
    const entry = this.entries.get(name);
    if (entry === undefined) {
      throw new OdyError(ErrorCodes.MCP_SERVER_NOT_FOUND, `Unknown MCP server: ${name}`);
    }
    if (entry.config.enabled === false) {
      throw new OdyError(ErrorCodes.MCP_SERVER_DISABLED, `MCP server is disabled: ${name}`);
    }
