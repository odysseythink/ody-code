...
const TEST_HOME = join(homedir(), '.ody-code-test');
...
async function readSessionLogs(sessionDir: string): Promise<{
  ...
  const files = (await readdir(logsDir))
    .filter((file) => file === 'ody-code.log' || file.startsWith('ody-code.log.'))
    .toSorted();
...
async function assertRotatedFilesStayBounded(sessionDir: string, files: readonly string[]): Promise<void> {
  assert(files.includes('ody-code.log.1'), 'session log did not rotate after exceeding max bytes');
  assert(!files.includes('ody-code.log.2'), 'session log kept more archives than configured');
...
async function main(): Promise<void> {
  process.env['ODY_CODE_HOME'] = TEST_HOME;
  process.env['ODY_LOG_LEVEL'] = 'warn';
  process.env['ODY_LOG_SESSION_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['ODY_LOG_SESSION_FILES'] = '2';
  process.env['ODY_LOG_GLOBAL_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['ODY_LOG_GLOBAL_FILES'] = '2';
  ...
  const harness = new KimiHarness({
    identity: { userAgentProduct: 'ody-code-cli', version: '0.1.1' },
    homeDir: TEST_HOME,
  });
  ...
  try {
    ...
    await writeFile(
      ...
      [
        ...
        '  - logs/ody-code.log',
        '  - logs/ody-code.log.1',
        `  - ${summary.sessionDir}/logs/ody-code.log`,
        `  - ${summary.sessionDir}/logs/ody-code.log.1`,
        '',
        'Rotation evidence:',
        `  - should NOT appear in session logs: ${evictedEntry}`,
        `  - should appear in session logs: ${finalEntry}`,
        '  - ody-code.log.1 exists',
        '  - ody-code.log.2 does not exist',
        '',
        'Export evidence:',
        '  - logging-session.zip includes session logs and wire.jsonl',
        '  - logging-session.zip does not include logs/global/ody-code.log',
        '  - logging-with-global.zip includes only the active global log at logs/global/ody-code.log',
        '  - global rotated logs such as ~/.ody-code-test/logs/ody-code.log.1 are intentionally not bundled',
        '',
        'Redaction evidence:',
