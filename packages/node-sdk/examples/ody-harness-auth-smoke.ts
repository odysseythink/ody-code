...
const MANAGED_ODY_CODE_PROVIDER = 'managed:ody-code';
...
async function main(): Promise<void> {
  ...
  try {
    if (forceLogin) {
      await harness.auth.logout(MANAGED_ODY_CODE_PROVIDER);
    ...
    const status = await harness.auth.status(MANAGED_ODY_CODE_PROVIDER);
    const usage = await harness.auth.getManagedUsage(MANAGED_ODY_CODE_PROVIDER);
    ...
    if (config.providers[MANAGED_ODY_CODE_PROVIDER]?.oauth?.key !== 'oauth/ody-code') {
    ...
    if (!keepToken) {
      await harness.auth.logout(MANAGED_ODY_CODE_PROVIDER).catch(() => {});
    }
    await harness.close();
