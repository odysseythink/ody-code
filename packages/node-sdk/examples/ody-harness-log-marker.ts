...
Options:
  -s, --session <id>   Existing session id to resume and mark
      --home <dir>     Kimi home dir; defaults to ODY_CODE_HOME or ~/.ody-code
...
async function main(): Promise<void> {
  ...
  const resolvedHome = resolveOdyHome(options.homeDir);
  const harness = new KimiHarness({
    identity: { userAgentProduct: 'ody-code-cli', version: 'log-marker' },
