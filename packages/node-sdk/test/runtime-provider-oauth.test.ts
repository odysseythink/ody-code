...
function managedConfig(): OdyConfig {
  return {
    providers: {
      'managed:ody-code': {
        type: 'kimi',
...
async function resolveRuntimeProviderWithOAuth(options: {
  ...
  if (model === undefined) {
    throw new OdyError(ErrorCodes.CONFIG_INVALID, 'No model is selected.');
  }
  ...
  try {
    ...
    if (
      !(error instanceof OdyError && error.code === ErrorCodes.AUTH_LOGIN_REQUIRED)
    ) {
  ...
  it('returns request-scoped OAuth auth without storing the initial access token in provider config', async () => {
    ...
    const resolved = await resolveRuntimeProviderWithOAuth({
      ...
      resolveOAuthTokenProvider: (_providerName, oauthRef) => {
        expect(oauthRef).toEqual({ storage: 'file', key: 'oauth/ody-code' });
        return { getAccessToken };
      },
    });

    expect(resolved.providerName).toBe('managed:ody-code');
      ...
      resolveRuntimeProviderWithOAuth({
        ...
        resolveOAuthTokenProvider: () => ({
          getAccessToken: vi.fn().mockRejectedValue(
            new OdyError(ErrorCodes.AUTH_LOGIN_REQUIRED, 'not logged in'),
          ),
        }),
      }),
    ).rejects.toMatchObject({ code: 'auth.login_required' });
