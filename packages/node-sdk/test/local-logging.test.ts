...
const LOG_ENV_KEYS = [
  'ODY_LOG_LEVEL',
  'ODY_LOG_GLOBAL_MAX_BYTES',
  'ODY_LOG_GLOBAL_FILES',
  'ODY_LOG_SESSION_MAX_BYTES',
  'ODY_LOG_SESSION_FILES',
] as const;
...
describe('Local logging — harness integration', () => {
  it('writes session-tagged entries to session log only and untagged entries to global', async () => {
    ...
    const globalPath = join(homeDir, 'logs', 'ody-code.log');
    const sessionLogPath = join(summary.sessionDir, 'logs', 'ody-code.log');
  ...
  it('default export bundles session log only; no globalLogPath in manifest', async () => {
    ...
    expect(entries.has('logs/ody-code.log')).toBe(true);
    expect(entries.has('logs/global/ody-code.log')).toBe(false);
    expect(entries.get('logs/ody-code.log')!.toString('utf-8')).toContain(
      'session export marker',
    );
    expect(result.manifest.sessionLogPath).toBe('logs/ody-code.log');
    ...
    const manifest = JSON.parse(entries.get('manifest.json')!.toString('utf-8')) as Record<
      ...
      unknown
    >;
    expect(manifest['sessionLogPath']).toBe('logs/ody-code.log');
  ...
  it('default export includes rotated session log files without requiring active ody-code.log', async () => {
    const env = snapshotLogEnv();
    process.env['ODY_LOG_LEVEL'] = 'warn';
    process.env['ODY_LOG_SESSION_MAX_BYTES'] = '1024';
    process.env['ODY_LOG_SESSION_FILES'] = '2';
    try {
      const homeDir = await makeTempDir('kimi-log-home-');
      const workDir = await makeTempDir('kimi-log-work-');
      const harness = new KimiHarness({ identity: TEST_IDENTITY, homeDir });
      ...
      const sessionLogEntries = [...entries.keys()].filter(
        (entry) => entry === 'logs/ody-code.log' || entry.startsWith('logs/ody-code.log.'),
      );
      expect(sessionLogEntries.length).toBeGreaterThan(0);
      expect(sessionLogEntries).toContain('logs/ody-code.log.1');
      expect(entries.has('logs/global/ody-code.log')).toBe(false);
      if (entries.has('logs/ody-code.log')) {
        expect(result.manifest.sessionLogPath).toBe('logs/ody-code.log');
      } else {
        expect(result.manifest.sessionLogPath).toBeUndefined();
      }
  ...
  it('--include-global-log bundles global active and sets manifest field', async () => {
    ...
    const entries = readZipEntries(zipBuf);
    expect(entries.has('agents/main/wire.jsonl')).toBe(true);
    expect(entries.has('logs/global/ody-code.log')).toBe(true);
    expect(result.manifest.globalLogPath).toBe('logs/global/ody-code.log');
    // Global log carries entries that don't have a sessionId routed to a sink.
    expect(entries.get('logs/global/ody-code.log')!.toString('utf-8')).toContain(
      'untagged probe',
    );
  });
  ...
  it('--include-global-log bundles the active root global log path', async () => {
    ...
    const entries = readZipEntries(await readFile(result.zipPath));
    const globalLog = entries.get('logs/global/ody-code.log')!.toString('utf-8');
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'ody-code.log'));
  ...
  it('logs export flush failures without failing the export', async () => {
    ...
    try {
      ...
      const entries = readZipEntries(await readFile(result.zipPath));
      const sessionLog = entries.get('logs/ody-code.log')!.toString('utf-8');
      const globalLog = entries.get('logs/global/ody-code.log')!.toString('utf-8');
  ...
  it('uses the latest harness homeDir for global diagnostic logging', async () => {
    ...
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'ody-code.log'));
    const secondLog = await readFile(join(secondHome, 'logs', 'ody-code.log'), 'utf-8');
  ...
  it('checks that an empty session log directory does not get a log file', async () => {
    ...
    try {
      ...
      expect(logsDir).not.toContain('ody-code.log');
    } finally {
      restoreLogEnv(env);
    }
  });
