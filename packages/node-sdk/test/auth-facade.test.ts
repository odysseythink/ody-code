...
describe('KimiHarness.auth', () => {
  ...
  it('exposes a cached access token without refreshing auth state', async () => {
    await new FileTokenStorage(join(homeDir, 'credentials')).save('ody-code', freshToken());
...
`,
    ...
    expect(() => new KimiHarness({ homeDir, identity: TEST_IDENTITY })).toThrow(
      /Model "ody-code\/kimi-for-coding" must define a positive max_context_size/,
    );
  });
...
`,
    ...
    await expect(
      new FileTokenStorage(join(homeDir, 'credentials')).load('ody-code'),
    ).resolves.toBeUndefined();

    const text = await readFile(join(homeDir, 'config.toml'), 'utf-8');
    expect(text).not.toContain('managed:ody-code');
    expect(text).not.toContain('ody-code/kimi-for-coding');
  ...
  it('submitFeedback maps camelCase input to snake_case body and posts with bearer auth', async () => {
    ...
    const result = await harness.auth.submitFeedback({
      content: 'great tool',
      sessionId: 'sess-42',
      version: 'ody-code-0.1.1',
      os: 'Darwin 25.3.0',
      model: 'ody-code/kimi-for-coding',
    });
