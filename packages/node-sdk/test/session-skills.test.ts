...
describe('Session skills', () => {
  it('lists session skills without exposing content', async () => {
    ...
    try {
      ...
      expect(listed?.path.endsWith('/.ody-code/skills/review/SKILL.md')).toBe(true);
  ...
  it('resolves user skills from the OS home directory, independently of ODY_CODE_HOME', async () => {
    const homeDir = await makeTempDir(tempDirs, 'kimi-sdk-skills-home-');
