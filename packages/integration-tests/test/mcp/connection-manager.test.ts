...
import { testKaos } from '@odysseythink/agent-core/test/helpers';
import type { ProviderConfig } from '@odysseythink/kosong';
...
import { ProviderManager } from '@odysseythink/agent-core';
...
import type { AgentEvent, SDKSessionRPC } from '@odysseythink/agent-core';
import { Session } from '@odysseythink/agent-core';
import { SessionAPIImpl } from '@odysseythink/agent-core';
import { createScriptedGenerate } from '@odysseythink/agent-core/test/helpers';
...
describe('Session MCP startup', () => {
  it('stores default MCP OAuth credentials under the configured Kimi home', async () => {
    ...
    try {
      ...
      await expect(
        readFile(
          join(processHome, '.ody-code', 'credentials', 'mcp', `${provider.storeKey}-tokens.json`),
          'utf-8',
        ),
