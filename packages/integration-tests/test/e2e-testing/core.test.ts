...
import { parseConfigString } from '@odysseythink/agent-core';
import { E2EConfigResolver } from '@odysseythink/e2e-testing';
import type { ResolvedE2EConfig } from '@odysseythink/e2e-testing';
import type { OdyConfig } from '@odysseythink/agent-core-shared';
import { ImpactAnalyzer } from '@odysseythink/e2e-testing';
import { registry } from '@odysseythink/e2e-testing';
import { TypeScriptVitestGenerator } from '@odysseythink/e2e-testing';
...
import type { AffectedTool, E2EPriority, Feature, ImpactAnalysisResult, ProjectStructure, TestFile } from '@odysseythink/e2e-testing';
...
describe('E2EGeneratorRegistry + TS/Vitest detection', () => {
  ...
  it('detectAndGet returns generator for agent-core project root', async () => {
    const gen = await registry.detectAndGet(REPO_ROOT);
  ...
  it('detectProjectStructure returns structure for agent-core', async () => {
    const gen = new TypeScriptVitestGenerator();
    const result = await gen.detectProjectStructure(join(REPO_ROOT, 'packages/agent-core'));
