...
describe('paths', () => {
  ...
  it('migratedMarker is under source', () => {
    expect(paths.migratedMarker('/x/.kimi')).toBe(join('/x/.kimi', '.migrated-to-ody-code'));
  });

  it('skipMarker is under target', () => {
    expect(paths.skipMarker('/y/.ody-code')).toBe(join('/y/.ody-code', '.skip-migration-from-kimi-cli'));
  });
