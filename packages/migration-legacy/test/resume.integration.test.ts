 ...
 *     all from `@odysseythink/agent-core/session/store`.
 *   - `Session` (constructor + `resume()` + `agents` map), from
 *     `@odysseythink/agent-core`; `localKaos` from `@odysseythink/kaos`. After
...
import {
  ...
  encodeWorkDirKey,
  normalizeWorkDir,
} from '@odysseythink/agent-core/session/store';
import { Session, type SDKSessionRPC } from '@odysseythink/agent-core';
