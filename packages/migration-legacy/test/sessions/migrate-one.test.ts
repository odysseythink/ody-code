...
describe('migrateOneSession (tiny-hello-world fixture)', () => {
  ...
  it('reports conflict when an unrelated ody-code session occupies the dir', async () => {
    ...
    // Overwrite state.json with a non-migrated (real) ody-code session.
  ...
  it('re-migrates a target dir left half-written by an interrupted run', async () => {
    ...
    // real ody-code session, so it must be re-migrated, not reported as a
  ...
  it('re-migrates a target dir whose state.json is corrupt', async () => {
    ...
    // is `ses_<uuid>`), not a real ody-code session, so it must be
