...
const MARKER_PATH = join(SOURCE_HOME, '.migrated-to-ody-code');
...
describe('runMigration (end-to-end on multi-workdir fixture)', () => {
  ...
  it('does not copy OAuth credentials into the target', async () => {
    ...
    try {
      ...
      await expect(
        readFile(join(tgt, 'credentials', 'ody-code.json'), 'utf-8'),
      ).rejects.toThrow();
      // The report tells the user to sign in again in ody-code.
      expect(report.notices.oauthLoginsRequiringRelogin).toContain('ody-code.json');
    } finally {
      await rm(src, { recursive: true, force: true });
    }
  });
