...
// ody-code's tui.toml `theme` enum (mirrors apps/ody-code TuiThemeSchema).
...
// The config.toml top-level keys ody-code understands, derived from the live
// KimiConfigSchema so the set tracks ody-code automatically. `raw` is internal
...
export interface ConfigStepResult {
  ...
  /** Top-level keys dropped because ody-code's config schema lacks them. */
  ...
  /** Count of kimi-cli hook entries dropped because ody-code's schema rejects them. */
...
/** True when the kimi-cli provider entry validates against ody-code's schema. */
...
/** True when the kimi-cli model entry validates against ody-code's schema. */
...
export async function migrateConfigStep(input: ConfigStepInput): Promise<ConfigStepResult> {
  ...
  // 1) Providers — keep only those ody-code's schema accepts.
  ...
  // 2) Models — keep only those ody-code's schema accepts, and not those
  ...
  // 2b) Hooks — keep only entries ody-code's HookDefSchema accepts. kimi-cli
  //     and ody-code share an identical hook shape, so a valid legacy hook
  //     passes straight through; the per-entry filter only guards against
  //     future schema drift (an event type / field ody-code does not know).
  ...
  // 4) Build the migrated top-level — only keys ody-code's schema supports.
  ...
  for (const [k, v] of Object.entries(parsed)) {
    ...
    if (k === 'default_yolo') {
      // kimi-cli's `default_yolo` maps to ody-code's `default_permission_mode`.
  ...
  // 4b) Drop any supported top-level key whose VALUE ody-code's config
