...
/**
 * Copy the user's legacy skills tree (~/.kimi/skills/) into ody-code's
 * default user skills root (~/.ody-code/skills/). Granularity is one
...
export async function migrateSkillsStep(input: SkillsStepInput): Promise<SkillsStepResult> {
  ...
  for (const name of entries) {
    ...
    // in it — touching it earlier would fail when ~/.ody-code/skills is
